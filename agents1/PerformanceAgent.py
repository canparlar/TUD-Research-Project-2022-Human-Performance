    ...
    def decide_on_bw4t_action(self, state: State):
        ...
        if (self._responseSpeed != 800) and ((self._second - self._responseSpeed) > 10.5):
            if self._lateResponse <= 0:
                ...
                self._sendMessage("You still haven't given an answer to my question, please do, so I can proceed.",
                                  'RescueBot')
            else:
        ...
        if self._noSuggestions > 0:
            ...
            self._sendMessage('You ignored me ' + str(self._trustValue) + ' ' + str(self._trustPhase) + ' ' + str(
                self._ignored) + ' ' + str(self._noSuggestions) + " trustAgentVarPresent " + str(
                state['rescuebot']['ignored']), 'RescueBot')
        workloadRound = round(self._workloadVal, 2)
        self._sendMessage(
            'Your workload is ' + str(round(self._workload[0], 2)) + ' ' + str(round(self._workload[1], 2))
            ...
            if Phase.FOLLOW_PATH_TO_ROOM == self._phase:
                ...
                if self._door['room_name'] in self._searchedRooms and self._goalVic not in self._foundVictims:
                    ...
                    if self._goalVic in self._foundVictims and str(self._door['room_name']) == \
                        ...
                        self._sendMessage(
                            'Moving to ' + str(self._door['room_name']) + ' to pick up ' + self._goalVic + '.' + self._reminderMessage,
                            'RescueBot')
                    if self._goalVic not in self._foundVictims and not self._remove or not self._goalVic and not self._remove:
                        self._sendMessage(
                            'Moving to ' + str(self._door['room_name']) + ' because it is the closest unsearched area.' + self._reminderMessage,
                            'RescueBot')
                    ...
                    if 'critical' in self._recentVic:
                        if not state[{'is_human_agent': True}]:
                            if self._lateComply <= 0:
                                ...
                                self._sendMessage('Please come to ' + str(self._door['room_name']) + ' to carry ' + str(
                                    self._recentVic) + ' together, a bit faster than last time, because we lost '
                                                       'important time then.', 'RescueBot')
                            else:
                                self._sendMessage('Please come to ' + str(self._door['room_name']) + ' to carry ' + str(
                                    self._recentVic) + ' together. You have to be quicker this time, because I am '
                                                       'waiting for you.', 'RescueBot')
                            self._waitingHuman = True
                            self._complySpeed = state['World']['tick_duration'] * state['World']['nr_ticks']
                        if state[{'is_human_agent': True}]:
